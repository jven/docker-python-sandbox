...
class PoolManager {
  ...
  _initializeContainer (cb) {
    const newPort = 32000 + Math.floor(Math.random() * 18000);
    this.options.containerLaunchOptions['HostConfig']['PortBindings']['3000/tcp'][0]['HostPort']
        = '' + newPort;
  ...
  _getContainerInfo(container, cb) {
    container.instance.inspect( (err, data) => {
      if (err
          ...
          || !data.NetworkSettings.Ports['3000/tcp']
          || !data.NetworkSettings.Ports['3000/tcp'].length
          || !data.NetworkSettings.Ports['3000/tcp'][0]['HostPort']) {
            
        err = err || new Error("unable to retrieve container host port")
        return container.cleanup(_.partial(cb, err))
      }
